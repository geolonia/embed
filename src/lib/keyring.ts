...
class Keyring {
  ...
  public parse(doc: Document = document) {
    ...
    if (this.#apiKey && this.#stage) {
      ...
      if (!this.#apiKey && !this.#stage) {
        throw new Error('Cannot load API key and stage. Run geolonia.setApiKey() and geolonia.setStage() manually.');
      } else if (this.#apiKey && !this.#stage) {
        throw new Error('Cannot load stage. Run and geolonia.setStage() manually.');
      } else { // if (!this.#apiKey && this.#stage)
        throw new Error('Cannot load API key. Check if API key is set or run geolonia.setApiKey() manually.');
      }
    }
  }
}
